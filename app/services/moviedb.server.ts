...
async function makeRequest<T>({ method, searchParams = new URLSearchParams(), path }: GetRequestParams): Promise<T> {
...
export const search = async (params: SearchParams = DEFAULT_PARAMS): Promise<MultiSearchResult> => {
    const searchParams = new URLSearchParams();
    Object.entries({ ...DEFAULT_PARAMS, ...params }).forEach(([key, value]) => {
        searchParams.set(key, String(value));
    });
    return makeRequest<MultiSearchResult>({
        method: RequestMethod.GET,
        path: MULTI_SEARCH,
        searchParams
    });
